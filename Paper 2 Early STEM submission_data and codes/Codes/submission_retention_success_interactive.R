...
rm(list=ls()) # clear the environment from previious codes
...
setwd("/Users/Prashant/000 STEM ED Paper 2 codes");
...
# Remove unused userdata 
df_limited <- df_original %>% select(-c(studentNum, Name, County, Country, State, In_State, HS_ATP_Code, HS_Name, AltHS_Name, High_Needs_Calculation1, High_Needs_Calculation2, High_Needs_Calculation1.1, High_Needs_Calculation2.1, High_Needs))

# Remove the data M0 and P0
df_limited <- df_limited[which(!(df_limited$Math_Score=="NA")),]
df_limited <- df_limited[which(!(df_limited$Physics_Score=="NA")),]

# choose cohort here
first_year<-2006
last_year<-2015
total_years<-last_year-first_year+1
df_limited <- df_limited[which(df_limited$Primary_Cohort <= last_year),]
# Remove the empty values, and force everything as numeric variables
df_integer <- df_limited[which(df_limited$Primary_Cohort==as.integer(df_limited$Primary_Cohort)),]
df_ENG <-df_integer[which(df_integer$Acad_Prog=='ENG'),] # Selecting ONLY Engineering students
df_ENG <-df_ENG[which(df_ENG$Ethnicity!='International'),] # We need to remove the Interenational students

#=========================Selection of subset of students ==========================

# the next line gives the high risk students

highrisk.choice<-readline(prompt="Do you want to choose ONLY the high-risk students? Y/N: ")

highrisk.choice<-substr(tolower(highrisk.choice), 1, 1) # select ONLY the first letter of the reply
if (highrisk.choice!="y" & highrisk.choice!="n"){
  highrisk.choice<-"n"}
# print(paste("highrisk.choice = ",toupper(highrisk.choice)))

if (highrisk.choice=="y")
  {
    df_1 <- df_ENG[which(df_ENG$M_minus_P_minus==1 | df_ENG$M_minus_P_minus_plus==1),]
    # print(paste("We have chosen the high-risk students only."))
    lowmed.choice<-"n"
}else
  {
    lowmed.choice<-readline(prompt="Do you want to choose ONLY the low/medium risk students? Y/N: ")
    lowmed.choice<-substr(tolower(lowmed.choice), 1, 1) # select ONLY the first letter of the reply
    if (lowmed.choice!="y" & lowmed.choice!="n"){
      lowmed.choice<-"n"}
    # print(paste("lowmed.choice = ",lowmed.choice)) 
    if (lowmed.choice=="y"){
      df_1 <- df_ENG[which(!(df_ENG$M_minus_P_minus==1 | df_ENG$M_minus_P_minus_plus==1)),]
      print(paste("We have chosen BOTH students with ONLY the low/medium risk levels."))
      }
    else{
      df_1<-df_ENG
      # print(paste("We have chosen BOTH students with all risk levels."))
      }
    # print(paste("We have chosen BOTH high-risk and non high-risk students."))
  }
rm(df_ENG) # remove the unused variable


#=========================Select or not select minority students ==========================


minority.choice<-readline(prompt="Do you want to choose ONLY the minority students? Y/N: ")
minority.choice<-substr(tolower(minority.choice), 1, 1) # select ONLY the first letter of the reply
if (minority.choice!="y" & minority.choice!="n"){
  minority.choice<-"n"}

# print(paste("minority.choice = ",minority.choice)) 
majority.choice<-"n"
if (minority.choice=="y")
{
  df_2 <- df_1[which(df_1$Minority==1),]
  # print(paste("We have chosen the minority students only."))
  majority.choice<-"n"
}else
{
  df_2<-df_1 # BOTH minority and majority included
  majority.choice<-readline(prompt="Do you want to choose ONLY the majority students? Y/N: ")
  majority.choice<-substr(tolower(majority.choice), 1, 1) # select ONLY the first letter of the reply
  if (majority.choice!="y" & majority.choice!="n"){
    majority.choice<-"n"}
  # print(paste("majority.choice = ",majority.choice)) 
  if (majority.choice=="y"){
    df_2 <- df_1[which(df_1$Majority==1),] # ONLY majority
    # print(paste("We have chosen ONLY majority students."))
  }else{
    df_2 <- df_1
    # print(paste("We have chosen BOTH majority and minority students."))
  }
    
}
rm(df_1) # remove the unused variable



#____________________Select or not select male/female students 


female.choice<-readline(prompt="Do you want to choose ONLY the female students? Y/N: ")
female.choice<-substr(tolower(female.choice), 1, 1) # select ONLY the first letter of the reply
# print(paste("female.choice = ",female.choice))

if (female.choice=="y")
{
  df_3 <- df_2[which(df_2$Sex==0),]
  # print(paste("We have chosen the female students only."))
  male.choice<-"n"
}else{
  
    df_3<-df_2 # Both male and female
    male.choice<-readline(prompt="Do you want to choose ONLY the male students? Y/N: ")
    male.choice<-substr(tolower(male.choice), 1, 1) # select ONLY the first letter of the reply
    # print(paste("male.choice = ",male.choice))

    if (male.choice=="y")
    {
      df_3 <- df_2[which(df_2$Sex==1),] # ONLY males
      # print(paste("We have chosen the female students only."))
    }else
    {
      df_3<-df_2 # BOTH male and females
    # print(paste("We have chosen BOTH male and female students."))
    }
}
rm(df_2) # remove the unused variable

#_____________FINAL SELECTION _________________

df_select<-df_3
rm(df_3, df_integer, df_original, df_limited) # remove the unused variable

#_____________Statements of what set we are studying______________________

if (highrisk.choice=="y")
  {print(paste("This study consists of ONLY the high risk students."))}

if (lowmed.choice=="y")
  {print(paste("This study consists of ONLY the low and medium risk students."))}

if (highrisk.choice=="n" & lowmed.choice=="n")
  {print(paste("This study consists of students at ALL risk levels."))}

if (minority.choice=="y")
  {print(paste("This study consists of ONLY minority students."))}

if (majority.choice=="y")
  {print(paste("This study consists of ONLY majority students."))}

if (minority.choice=="n" & majority.choice=="n")
  {print(paste("This study consists of students of ALL races/ethnicities."))}

if (female.choice=="y")
  {print(paste("This study consists of ONLY female students."))}


if (male.choice=="y")
  {print(paste("This study consists of ONLY male students."))}

if (female.choice=="n" & (male.choice=="n"))
  {print(paste("This study consists of BOTH male and female students."))}
...
df_treatment <- df_select[which(df_select$ES110_in_1st_semester==1 & df_select$PH131_in_1st_semester==0 & df_select$PH131_in_1st_year==1 &  df_select$Primary_Cohort>=2011 & df_select$Clarkson_School==0),]

# The early control group is the group of high-risk ENG students before 2011 who did not take ES110 in sem 1
df_control <-df_select[which(df_select$ES110_in_1st_semester==0 & df_select$PH131_in_1st_semester==1 & df_select$Primary_Cohort<=2010 & df_select$Clarkson_School==0),]
...
#===============================================================================================================================
# combine the control group in 2006-2010 with treatment group (2011-2015) of the ENG student
...
for (i in 1:total_years){
  ...
  #Delayed
  #df_visual$tobe_delayed_students_num[i]<-sum(df_year$Delayed)
  #df_visual$tobe_delayed_students_percent[i]<-signif(100*sum(df_year$Delayed)/df_visual$STEM_entry_num[i], 3)
...
pdf("female retention_plots.pdf",width=10,height=10)
...
yr2_plot <- ggplot(mapping = aes(x = X1, y = X2-40)) +
  ...
  annotate(geom = "text", x = 2009.7, y = 65, label = "p", hjust = "left", family="Times", fontface="italic") +
  annotate(geom = "text", x = 2010.0, y = 65, label = "= .068", hjust = "left", family="Times") +
...
yr3_plot <- ggplot(mapping = aes(x = X1, y = X2-40)) +
  ...
  annotate(geom = "text", x = 2009.7, y = 65, label = "p", hjust = "left", family="Times", fontface="italic") +
  annotate(geom = "text", x = 2010.0, y = 65, label = "= .237", hjust = "left", family="Times") +
  scale_x_discrete(limits=first_year:last_year) +
  scale_y_continuous(limits= c(0,65),
                     labels = c("40", "60","80","100"))+
...
plot_4yr <- ggplot(mapping = aes(x = X1, y = X2-40)) +
  ...
  annotate(geom = "text", x = 2009.7, y = 65, label = "p", hjust = "left", family="Times", fontface="italic") +
  annotate(geom = "text", x = 2010.0, y = 65, label = "= .083", hjust = "left", family="Times") +
  scale_x_discrete(limits=first_year:last_year) +
  scale_y_continuous(limits= c(0,65),
                     labels = c("40", "60","80","100"))+
...
plot_6yr <- ggplot(mapping = aes(x = X1, y = X2-40)) +
  ...
  annotate(geom = "text", x = 2009.7, y = 65, label = "p", hjust = "left", family="Times", fontface="italic") +
  annotate(geom = "text", x = 2010.0, y = 65, label = "= .374", hjust = "left", family="Times") +
  scale_x_discrete(limits=first_year:last_year) +
  scale_y_continuous(limits= c(0,65),
                     labels = c("40", "60","80","100"))+
...
pdf("female success_rates.pdf",width=12,height=15)
...
plot_PH131 <- ggplot(mapping = aes(x = X1, y = X2-40)) +
  ...
  annotate(geom = "text", x = 2009.7, y = 60, label = "p", hjust = "left", family="Times", size=5.0, fontface="italic") +
  annotate(geom = "text", x = 2010.0, y = 60, label = "= .002**", hjust = "left", size=5.0, family="Times") +
  scale_x_discrete(limits=first_year:last_year) +
...
plot_PH132 <- ggplot(mapping = aes(x = X1, y = X2-40)) +
  ...
  annotate(geom = "text", x = 2009.7, y = 60, label = "p", hjust = "left", size=5.0, family="Times", fontface="italic") +
  annotate(geom = "text", x = 2010.0, y = 60, label = "= .653", hjust = "left", size=5.0, family="Times") +
  scale_x_discrete(limits=first_year:last_year) +
  scale_y_continuous(limits= c(0,60),
                     labels = c("40", "60","80","100"))+
...
plot_MA131 <- ggplot(mapping = aes(x = X1, y = X2-40)) +
  ...
  annotate(geom = "text", x = 2009.7, y = 60, label = "p", hjust = "left", size=5.0, family="Times", fontface="italic") +
  annotate(geom = "text", x = 2010.0, y = 60, label = "= .030*", hjust = "left", size=5.0, family="Times") +
  scale_x_discrete(limits=first_year:last_year) +
  scale_y_continuous(limits= c(0,60),
                     labels = c("40", "60","80","100"))+
...
plot_MA132 <- ggplot(mapping = aes(x = X1, y = X2-40)) +
  ...
  annotate(geom = "text", x = 2009.7, y = 60, label = "p", hjust = "left", size=5.0, family="Times", fontface="italic") +
  annotate(geom = "text", x = 2010.0, y = 60, label = "= .863", hjust = "left", size=5.0, family="Times") +
  scale_x_discrete(limits=first_year:last_year) +
  scale_y_continuous(limits= c(0,60),
                     labels = c("40", "60","80","100"))+
...
plot_CM131 <- ggplot(mapping = aes(x = X1, y = X2-40)) +
  ...
  annotate(geom = "text", x = 2009.7, y = 62, label = "p", hjust = "left", size=5.0, family="Times", fontface="italic") +
  annotate(geom = "text", x = 2010.0, y = 62, label = "= .249", hjust = "left", size=5.0, family="Times") +
  scale_x_discrete(limits=first_year:last_year) +
...
plot_CM132 <- ggplot(mapping = aes(x = X1, y = X2-42)) +
  ...
  annotate(geom = "text", x = 2010.7, y = 63, label = "p", hjust = "left", size=5.0, family="Times", fontface="italic") +
  annotate(geom = "text", x = 2011.0, y = 63, label = "= .880", hjust = "left", size=5.0, family="Times") +
  scale_x_discrete(limits=first_year:last_year) +
  scale_y_continuous(limits= c(0,63),
                     labels = c("40", "60","80","100"))+
